...
def ziskat_api_odpoved(url):
    odpoved = requests.get(url)
    status = odpoved.status_code
    return status

# Vraci json data
def ziskat_api_data(url):
    odpoved = requests.get(url)
    data = odpoved.json()
    return data
...
def ulozit_do_json_souboru(nazev_souboru, data):
...
def filtr_postu_podle_user_id(seznam_postu, user_id=''):
    if user_id == '':
